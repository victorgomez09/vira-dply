<script lang="ts">
	...
	function parseLogLine(line: string): Omit<LogEntry, 'id'> | null {
		...
		const source = application?.name || 'application';
	...
	async function startLogStreaming() {
		...
		stopStreaming = await applicationsApi.streamLogs(
			projectId,
			resId,
			true,
			(line) => {
	...
	function downloadLogs() {
		...
		a.href = url;
		a.download = `${application?.name || 'application'}-logs-${new Date().toISOString()}.txt`;
...
{#if application}
	<div class="space-y-6">
		<div class="flex items-center justify-between">
			<div>
				<h2 class="text-2xl font-bold tracking-tight">Application Logs</h2>
				<p class="text-muted-foreground">Real-time logs for {application.name}</p>
			</div>
