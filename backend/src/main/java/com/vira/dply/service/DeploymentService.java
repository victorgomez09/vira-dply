...
public class DeploymentService {
    ...
    public void deployApplication(ApplicationEntity app, String imageTag) {
        ...
        DeploymentBuilder deploymentBuilder = new DeploymentBuilder()
                ...
                .withImage(imageTag)
                .addNewPort().withContainerPort(containerPort).endPort() // <-- solo aquí
                .endContainer()
                .endSpec()
                .endTemplate()
                .endSpec();
        ...
        KubernetesClient client = kubernetesClientProvider.getClientForEnvironment(app.getProject().getEnvironment());
    ...
    private void createHttpRoutes(ApplicationEntity app, String namespace, Set<DomainEntity> domains, KubernetesClient client) {
        for (DomainEntity domain : domains) {
            ...
            spec.put("parentRefs", List.of(Map.of("name", domain.getGatewayType().name().toLowerCase() + "-gateway")));
            ...
            spec.put("rules", List.of(Map.of(
                    ...
                    "forwardTo", List.of(Map.of("serviceName", app.getName(), "port", domain.getPort())))));

            if (Boolean.TRUE.equals(domain.getTls())) {
                spec.put("tls", Map.of("secretName", app.getName() + "-tls-" + domain.getHost().replace(".", "-")));
            }
            ...
            log.info("HTTPRoute created for domain {} in namespace {} using gateway {}", domain.getHost(), namespace,
                    domain.getGatewayType());
        }
    }

    private void createCertificate(ApplicationEntity app, String namespace, DomainEntity domain, KubernetesClient client) {
